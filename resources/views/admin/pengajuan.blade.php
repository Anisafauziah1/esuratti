    ...
    <aside id="sidebar"
        ...
        <nav class="flex flex-col mt-6 space-y-4 px-6 text-center">
            <a href=" {{ route('admin.homeadmin') }}"
                class="text-[#78977F] font-semibold text-base py-2 rounded hover:bg-[#78977F] hover:text-white transition">
                Beranda
            </a>
                    ...
                    <tbody>
                        @foreach ($pengajuans as $pengajuan)
                            <tr class="border-b border-gray-200 text-sm text-black">
                                <td class="py-4 px-6">{{ $pengajuan->user->name }}</td>
                                <td class="py-4 px-6">{{ $pengajuan->user->nim }}</td>
                                <td class="py-4 px-6">{{ $pengajuan->suratType->nama }}</td>
                                <td class="py-4 px-6">{{ $pengajuan->created_at->format('d M Y') }}</td>
                                <td class="py-4 px-6">
                                    <span
                                        class="
                {{ $pengajuan->status === 'accepted' ? 'text-green-700' : '' }}
                {{ $pengajuan->status === 'pending' ? 'text-yellow-600' : '' }}
                {{ $pengajuan->status === 'rejected' ? 'text-red-700' : '' }}
            ">
